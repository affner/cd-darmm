...
Private Sub ACOUNT_BOX_Change()
    ...
    ' Cambiar las opciones según la selección
...
'INICIALIZAR VARIABLES
    ' Definir criterios según el mercado seleccionado, NS (Nombre Sheet)
    Select Case MARKET_BOX.Value
    ...
    ' Definir criterios según el account seleccionado
    Select Case ACOUNT_BOX.Value
    ...
    ' Definir criterios según el activo seleccionado
...
' FECHA
    'Dice el día de la fecha y el DR (Días Restantes)
...
' LIMPIEZA COMPLETA DE LA HOJA MARKETS Y REVISIÓN DE DÍAS
    If DR = 0 Then
        ...
        ' Hojas que no se eliminarán
        HOJASBASE = Array("RESULTS", "BD_MARKET", "MARKETS", "PLANTILLA")
        ' Recorremos todas las hojas
        Application.DisplayAlerts = False ' Desactivar alertas para evitar mensajes de confirmación
        For Each WS In ThisWorkbook.Worksheets
            ' Comprobamos si la hoja no está en la lista de hojas excluidas
        ...
        Else
            'Cambia la fecha por día y solo pueden existir # fechas diferentes dependiendo el día que inicie
    ...
    For i = 1 To Application.WorksheetFunction.Min(LRMA, LRMB, LRMC)
        ...
        If WSRM.Cells(i, 1).Value = CRITERIA_MARKET And _
           ...
           WSRM.Cells(i, 3).Value = CRITERIA_MARKETDATA Then
            ...
            RESPUESTA = MsgBox("Error, se repite la información. ¿Desea reemplazar los datos?", vbYesNo + vbExclamation, "Reemplazar datos")
            B3 = False
            If RESPUESTA = vbNo Then
                ' Si el usuario elige NO, salir del Sub
                Exit Sub
            Else
                ' Si el usuario elige SÍ, reemplazar los datos
...
' CREAR HOJA CRITERIA_MARKET
    ...
    If Not B2 Then
        ...
        ' Copiar controles (botones, imágenes, etc.) de la hoja origen
        ' Copiar las formas (botones, imágenes, etc.) de la hoja origen
        For Each shp In WSP.Shapes
            shp.Copy
            Worksheets(CRITERIA_MARKET).Paste
        Next shp
        ' Limpiar el portapapeles para evitar el mensaje de "¿Deseas mantener los datos copiados?"
        ...
        If Worksheets(CRITERIA_MARKET).Cells(1, i).Value = CRITERIA_MARKET And _
           ...
           Worksheets(CRITERIA_MARKET).Cells(3, i).Value = CRITERIA_MARKETDATA Then
            ...
            If RESPUESTA = vbNo Then
                ...
                Exit Sub
            Else
                ' Si el usuario elige SÍ, reemplazar los datos en la hoja CRITERIA_MARKET
